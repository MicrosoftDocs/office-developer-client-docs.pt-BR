...
## <a name="features-supported-by-both-infopath-and-infopath-forms-services"></a>Recursos suportados pelo ambos InfoPath e o InfoPath Forms Services

As seções a seguir listam os recursos que são compatíveis com os modelos de formulário compatíveis com navegador implantados no InfoPath Forms Services que pode ser aberto no InfoPath e o navegador.
...
> [!NOTE]
> Regras simples, cálculos e validação de dados são habilitadas e executados no navegador usando o JScript. Regras complexas, cálculos e validação de dados exigem um postback para realizar essas operações no servidor. 
  
### <a name="code"></a>Código

Código da lógica de negócios deve ser baseado no modelo de objeto de código gerenciado do InfoPath fornecido pelo namespace [Microsoft.Office.InfoPath](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.aspx) . Código de lógica de negócios em execução no servidor está sujeito às seguintes restrições: 
  
- Como cada solicitação de servidor pode ser manipulada por um front-end diferente e InfoPath Forms Services carregará apenas uma instância da lógica de negócios, programadores não podem confiar nos dados armazenados nas variáveis globais ou estáticas. Para acomodar isso, a lógica de negócios deve armazenar o estado em um recipiente de propriedades, acesso para o qual é fornecido pela propriedade [FormState](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.XmlForm.FormState.aspx) . 
    
- Um subconjunto dos membros do namespace **Microsoft.Office.InfoPath** fornecem recursos, como o gerenciamento de direitos de informação (IRM), que não são suportados no servidor. Para obter mais informações em que os membros do modelo de objeto são e não são suportados, consulte as seções "Objeto modelo membros que trabalho no InfoPath e InfoPath Forms Services" e "Objeto modelo membros que trabalho somente no InfoPath" neste tópico. 
    
- Lógica comercial gravada no VBScript, JScript e o modelo de objeto compatível com o InfoPath 2003 fornecidos por membros do namespace [SemiTrust](https://msdn.microsoft.com/library/Microsoft.Office.Interop.InfoPath.SemiTrust.aspx) não é suportada no servidor. 
    
## <a name="features-not-supported-by-infopath-forms-services"></a>Recursos não suportados pelo InfoPath Forms Services

As seções a seguir listam os recursos que não são compatíveis com os modelos de formulário compatíveis com navegador implantados no InfoPath Forms Services que pode ser aberto no InfoPath e o navegador.
  
Ao usar o recurso de **Verificador de Design** no modo de design do InfoPath para confirmar a compatibilidade com o InfoPath Forms Services, recursos que não são suportados causarão erros ou mensagens. Recursos que produzem erros impedirá o modelo de formulário que está sendo publicado como um formulário habilitado para navegador. Recursos que geraram mensagens são permitidos, mas esse recurso específico não será executado quando o formulário é aberto em um navegador. 
  
### <a name="controls"></a>Controles

Os seguintes controles e recursos de controle não são suportados nos modelos de formulário que podem ser abertos InfoPath e o navegador.
  
- Filtros nos controles de repetição
...
### <a name="other-features-that-are-not-supported-or-not-fully-supported-by-infopath-forms-services"></a>Outros recursos que são incompatíveis ou não totalmente suportado pelo InfoPath Forms Services
...
- Conexões de dados do banco de dados são limitadas para acesso somente leitura aos bancos de dados do SQL server
    
- Funções de usuário
    
- Extensibilidade de assinatura digital através do modelo de objeto. Há suporte para assinar digitalmente no servidor por meio de um controle ActiveX que é executado apenas no Microsoft Internet Explorer.
    
- Integração de Human Workflow Services (HWS). HWS foi substituída pelo BizTalk server
    
- Substitui a mensagem de erro de esquema XML. Esse é um recurso raramente usado que permite que o designer de formulários fornecer uma mensagem diferente daquela fornecida pelo MSXML ou **System. XML** quando um documento não valida (geralmente devido a uma incompatibilidade de tipo). Esse recurso não é suportado na interface do usuário designer e exige a edição de mão do formulário (. xsf) do arquivo de definição. 
    
### <a name="features-with-no-direct-parallel-on-the-infopath-forms-services"></a>Recursos com nenhum paralelo direto do InfoPath Forms Services
...
- Salvamento automático, detecção de falha e recuperação
    
- Envelope de mensagem
    
- Exportar para Excel
    
- Tablet / recursos de tinta, incluindo o controle de **Imagem de tinta** 
...
- Extensibilidade XSLT (**xd: preserve** blocos) 
...
> [!NOTE]
> Esses recursos não produzem notificações de qualquer mensagem de erro ou ao usar o recurso de **Verificador de Design** no modo de design do InfoPath. 
  
## <a name="object-model-members-that-work-in-both-infopath-and-infopath-forms-services"></a>Membros do modelo de objeto que funcionam em ambos os InfoPath e o InfoPath Forms Services

O InfoPath fornece um novo modelo de objeto de código gerenciado com um conjunto básico de funcionalidade para a criação de lógica de negócios personalizado em modelos de formulário. Quando implantado no SharePoint Server 2010 com o InfoPath Forms Services, criada usando esse novo modelo de objeto de lógica de negócios será executado em ambos os um navegador da Web e no InfoPath. Opcionalmente, você pode escrever a lógica de negócios que usa um nível adicional de funcionalidade disponível por meio desse modelo de objeto que será executado apenas nos modelos de formulário abertos para edição no InfoPath.
  
Para escrever lógica de negócios que será executado quando um formulário é aberto em ambos os um navegador da Web e no InfoPath, marque a caixa de seleção **Habilitar apenas a recursos compatíveis com navegador** , na caixa de diálogo **Design um modelo de formulário** , ao criar um novo modelo de formulário. Para escrever lógica de negócios que pode usar a funcionalidade adicional somente quando aberto no InfoPath, desmarque a caixa de seleção **Habilitar apenas a recursos compatíveis com navegador** ao criar um novo modelo de formulário. Você também pode alterar essa configuração após a criação de um modelo de formulário, clicando em **Alterar configurações de compatibilidade** no painel de tarefas **Verificador de Design** e, em seguida, marcando ou desmarcando o **o Design de um modelo de formulário que pode ser aberto em um navegador ou do InfoPath **caixa de seleção. Se você optar por criar um modelo de formulário compatíveis com navegador, o compilador exibirá um erro se você tiver usado a qualquer classes ou membros que não são compatíveis com o InfoPath Forms Services. 
  
> [!NOTE]
> Após a publicação de um modelo de formulário habilitados para navegador que contém código gerenciado para o SharePoint Server 2010 com o InfoPath Forms Services, ou em um local compartilhado, o modelo de formulário deve ser carregado e aprovado por um administrador do servidor antes de ele terão permissão para executar. 
  
As classes e os membros do modelo de objeto de código gerenciado do InfoPath fornecidos pelo namespace [Microsoft.Office.InfoPath](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.aspx) a seguir são suportados no InfoPath e o InfoPath Forms Services. 
...
||[Origem](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.ClickedEventArgs.Source.aspx) <br/> |
|[ControlEvents](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.ControlEvents.aspx) <br/> |[1.1](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.ControlEvents.Item.aspx) <br/> |
|[DataConnection](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.DataConnection.aspx) <br/> |[Execução](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.DataConnection.Execute.aspx) <br/> |
...
||[1.1](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.DataConnectionCollection.Item.aspx) <br/> |
||[1.1](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.DataConnectionCollection.Item.aspx) <br/> |
|[Fonte de dados](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.DataSource.aspx) <br/> |[CreateNavigator](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.DataSource.CreateNavigator.aspx) <br/> |
...
||[1.1](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.DataSourceCollection.Item.aspx) <br/> |
||[1.1](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.DataSourceCollection.Item.aspx) <br/> |
...
||[Cco](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.EmailSubmitConnection.Bcc.aspx) <br/> |
...
||[Execução](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.EmailSubmitConnection.Execute.aspx) <br/> |
||[Introduction](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.EmailSubmitConnection.Introduction.aspx) <br/> |
||[Assunto](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.EmailSubmitConnection.Subject.aspx) <br/> |
||[Para](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.EmailSubmitConnection.To.aspx) <br/> |
...
|[FileQueryConnection](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.FileQueryConnection.aspx) <br/> |[Execução](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.FileQueryConnection.Execute.aspx) <br/> |
||[FileLocation](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.FileQueryConnection.FileLocation.aspx) <br/> |
|[FileSubmitConnection](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.FileSubmitConnection.aspx) <br/> |[Execução](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.FileSubmitConnection.Execute.aspx) <br/> |
...
||[Excluir](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.FormErrorCollection.Delete.aspx) <br/> |
...
||[GetErrors](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.FormErrorCollection.GetErrors.aspx) <br/> |
||[1.1](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.FormErrorCollection.Item.aspx) <br/> |
...
||[SetDefaultView](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.LoadingEventArgs.SetDefaultView.aspx) <br/> |
|[SharepointListQueryConnection](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.SharepointListQueryConnection.aspx) <br/> |[Execução](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.SharepointListQueryConnection.Execute.aspx) <br/> |
...
|[ViewInfo](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.ViewInfo.aspx) <br/> |[Legenda](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.ViewInfo.Caption.aspx) <br/> |
...
||[Padrão](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.ViewInfoCollection.Default.aspx) <br/> |
||[GetEnumerator](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.ViewInfoCollection.GetEnumerator.aspx) <br/> |
||[Inicial](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.ViewInfoCollection.Initial.aspx) <br/> |
||[1.1](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.ViewInfoCollection.Item.aspx) <br/> |
||[1.1](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.ViewInfoCollection.Item.aspx) <br/> |
||[SwitchView](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.ViewInfoCollection.SwitchView.aspx) <br/> |
||[SwitchView](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.ViewInfoCollection.SwitchView.aspx) <br/> |
|[WebServiceConnection](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.WebServiceConnection.aspx) <br/> |[Execução](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.WebServiceConnection.Execute.aspx) <br/> |
...
||[Validando](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.XmlEvent.Validating.aspx) <br/> |
...
|[XmlEvents](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.XmlEvents.aspx) <br/> |[1.1](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.XmlEvents.Item.aspx) <br/> |
||[1.1](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.XmlEvents.Item.aspx) <br/> |
...
||[Conexões de dados](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.XmlForm.DataConnections.aspx) <br/> |
||[Fontes de dados](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.XmlForm.DataSources.aspx) <br/> |
...
||[Novo](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.XmlForm.New.aspx) <br/> |
...
|[XmlOperation](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.XmlOperation.aspx) <br/> |[Excluir](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.XmlOperation.Delete.aspx) <br/> |
||[Inserir](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.XmlOperation.Insert.aspx) <br/> |
...
As classes e os membros do modelo de objeto de código gerenciado do InfoPath fornecidos pelo namespace [Microsoft.Office.InfoPath](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.aspx) a seguir são suportados apenas no InfoPath. 
  
> [!NOTE]
> Esses membros do modelo de objeto podem ser usados no código de um modelo de formulário habilitados para navegador, se você escrever lógica condicional que determina se o formulário é aberto no navegador ou do InfoPath. Para obter mais informações, consulte [gravar lógica condicional que determina o ambiente de tempo de execução](how-to-write-conditional-logic-that-determines-the-run-time-environment.md). 
  
|**Classe pai**|**Membros**|
|:-----|:-----|
|[ActionType](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.ActionType.aspx) <br/> |[Cópia](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.ActionType.Copy.aspx) <br/> |
||[Recortar](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.ActionType.Cut.aspx) <br/> |
||[Excluir](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.ActionType.Delete.aspx) <br/> |
...
||[Expirou](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.CertificateStatus.Expired.aspx) <br/> |
...
||[Mesclar](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.FormEvents.Merge.aspx) <br/> |
||[Save](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.FormEvents.Save.aspx) <br/> |
||[Sinal](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.FormEvents.Sign.aspx) <br/> |
|[FormTemplate](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.FormTemplate.aspx) <br/> |[ID do cache](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.FormTemplate.CacheId.aspx) <br/> |
...
||[Cco](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.MailEnvelope.Bcc.aspx) <br/> |
...
||[Assunto](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.MailEnvelope.Subject.aspx) <br/> |
||[Para](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.MailEnvelope.To.aspx) <br/> |
...
||[Habilitado](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.Permission.Enabled.aspx) <br/> |
...
||[1.1](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.SignatureCollection.Item.aspx) <br/> |
...
||[Referenda](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.SignatureRelation.CounterSign.aspx) <br/> |
||[Simples](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.SignatureRelation.Single.aspx) <br/> |
...
|[SignedDataBlock](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.SignedDataBlock.aspx) <br/> |[Legenda](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.SignedDataBlock.Caption.aspx) <br/> |
||[Name](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.SignedDataBlock.Name.aspx) <br/> |
||[Sinal](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.SignedDataBlock.Sign.aspx) <br/> |
...
||[1.1](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.SignedDataBlockCollection.Item.aspx) <br/> |
...
||[1.1](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.TaskPaneCollection.Item.aspx) <br/> |
||[1.1](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.TaskPaneCollection.Item.aspx) <br/> |
...
||[Clip-art](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.TaskPaneType.ClipArt.aspx) <br/> |
||[Localizar](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.TaskPaneType.Find.aspx) <br/> |
...
||[Substituir](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.TaskPaneType.Replace.aspx) <br/> |
||[Verificar ortografia](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.TaskPaneType.Spelling.aspx) <br/> |
...
||[Remover](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.UserPermission.Remove.aspx) <br/> |
...
||[1.1](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.UserPermissionCollection.Item.aspx) <br/> |
...
|[Window](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.Window.aspx) <br/> |[Ativar](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.Window.Activate.aspx) <br/> |
||[Active](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.Window.Active.aspx) <br/> |
||[Legenda](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.Window.Caption.aspx) <br/> |
...
||[Altura](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.Window.Height.aspx) <br/> |
||[À esquerda](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.Window.Left.aspx) <br/> |
...
||[Início](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.Window.Top.aspx) <br/> |
||[Largura](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.Window.Width.aspx) <br/> |
...
||[1.1](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.WindowCollection.Item.aspx) <br/> |
...
||[Nome do host](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.XmlForm.HostName.aspx) <br/> |
...
||[Imprimir](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.XmlForm.Print.aspx) <br/> |
||[Imprimir](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.XmlForm.Print.aspx) <br/> |
||[Recuperados](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.XmlForm.Recovered.aspx) <br/> |
...
||[Item de](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.XmlFormCollection.Item.aspx) <br/> |
||[Novo](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.XmlFormCollection.New.aspx) <br/> |
||[Novo](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.XmlFormCollection.New.aspx) <br/> |
