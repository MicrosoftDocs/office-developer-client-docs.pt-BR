...
As regras do Outlook podem operar tanto do lado do servidor quanto do cliente, dependendo do tipo de conta e regra. Há várias maneiras de implementar regras para aplicar nos seus próprio esquemas organizacionais quando estiver organizando itens na caixa de correio. Por exemplo, você pode criar uma hierarquia de subpastas que organiza emails não lidos e emails lidos pela área de assunto. Ou você pode criar uma hierarquia de subpastas que correspondem ao remetente da mensagem. Você pode também categorizar seus emails e usar pastas de pesquisa para agregar os emails por categoria.
...
O objeto **regra**tem a propriedade [RuleType](https://msdn.microsoft.com/library/bb645613\(v=office.15\)) que indica se a regra é uma regra de envio ou recebimento. Quando uma regra é criada, a propriedade **RuleType** é especificada e não pode ser alterada sem excluir e recriar a regra com uma diferente propriedade **RuleType**. Os objetos [RuleAction](https://msdn.microsoft.com/library/bb644297\(v=office.15\)) e [RuleCondition](https://msdn.microsoft.com/library/bb612469\(v=office.15\)) e seus  conjuntos de objetos, ações derivadas e objetos de condição também são usados para mais suporte ao editar ações de regras e condições da regra.
...
1.  Obter o conjunto **regras** a partir da propriedade [DefaultStore](https://msdn.microsoft.com/library/bb623164\(v=office.15\)) do objeto[NameSpace](https://msdn.microsoft.com/library/bb645857\(v=office.15\)) ligando para o  método[GetRules()](https://msdn.microsoft.com/library/bb609979\(v=office.15\)) no objeto padrão[Store](https://msdn.microsoft.com/library/bb609139\(v=office.15\)). Use um bloqueio **try...catch** para conta de usuário que esteja offline ou desconectado do servidor Exchange. Isso impede que o Outlook gere um erro.

2.  Ligar para o método [criar (cadeia de caracteres, OlRuleType)](https://msdn.microsoft.com/library/bb643857\(v=office.15\)) no objeto **regras** para criar uma instância variável ou objeto**regra** especificando um nome e um parâmetro[ OlRuleType](https://msdn.microsoft.com/library/bb645776\(v=office.15\)).

3.  Use as coleções [RuleActions](https://msdn.microsoft.com/library/bb610113\(v=office.15\)) e [RuleConditions](https://msdn.microsoft.com/library/bb610965\(v=office.15\)) para habilitar exceções, condições e ações no objeto**regra**. Observe que qualquer condição habilitada no conjunto retornado**RuleConditions** pela propriedade [exceções](https://msdn.microsoft.com/library/bb609880\(v=office.15\)) será tratada como uma exceção à condição de regra de e outras ações personalizadas internas ou condições não poderão ser adicionar à coleção.

4.  Configurar a propriedade [habilitada](https://msdn.microsoft.com/library/bb609147\(v=office.15\)) como **verdadeira** para que cada regra de ação, condição ou exceção sejam operacionais. Algumas ações ou condições, assim como a propriedade[pasta](https://msdn.microsoft.com/library/bb646755\(v=office.15\)), exigem que você defina propriedades adicionais na ação ou na condição para salvar o objeto**regra** sem um erro.

5.  Por fim, chame o método [Save(Object)](https://msdn.microsoft.com/library/bb610738\(v=office.15\)) no conjunto**regras** para salvar regras criadas ou modificadas. Coloque o método**salvar** método em um bloqueio **try...catch ** para lidar com exceções.

No exemplo código a seguir, o CreateManagerRule implementa as etapas descritas anteriormente. Primeiro o CreateManagerRule verificar se a propriedade[CurrentUser](https://msdn.microsoft.com/library/bb622574\(v=office.15\))representa um objeto [ExchangeUser](https://msdn.microsoft.com/library/bb609574\(v=office.15\)) indicando que o usuário atual é um usuário do Exchange. Se o usuário atual é um usuário do Exchange, o CreateManagerRule receberá Gerenciador do usuário atual ligando para o método [GetExchangeUserManager()](https://msdn.microsoft.com/library/bb646656\(v=office.15\)) do objeto**ExchangeUser** da propriedade ** CurrentUser** do objeto **NameSpace**. Uma regra de recebimento é criada, em seguida, para mover as mensagens recebidas para uma subpasta da caixa de entrada de acordo com as seguintes condições:

- A mensagem é do gerente do usuário.

- O destinatário estivá na linha**para:** da mensagem.
