...
O layout de um cartão de visita pode ser representado como uma imagem e um número de campos de texto. A imagem pode ser uma foto de contato ou uma imagem de cartão. Os campos de texto consistem em um valor de outra propriedade definida no contato e uma cadeia de caracteres de rótulo personalizada opcional fornecida pelo usuário. Observe que os valores de vários bytes são armazenados no formato little-endian no buffer.
