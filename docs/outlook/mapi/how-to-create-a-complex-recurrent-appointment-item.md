...
O MAPI pode ser usado para criar itens de compromisso recorrentes.
...
2. Abra uma pasta Calendário no armazenamento de mensagens. Consulte **PR_IPM_APPOINTMENT_ENTRYID** ([PidTagIpmAppointmentEntryId](pidtagipmappointmententryid-canonical-property.md)).
...
4. Definir a **propriedade dispidApptRecur** ([PidLidAppointmentRecur](pidlidappointmentrecur-canonical-property.md)) e outras propriedades necessárias para criar um compromisso recorrente.
    
5. Salve o novo item de compromisso.
    
A  `AddAppointment` função no arquivo de origem Appointments.cpp do projeto CreateOutlookItemsAddin demonstra estas etapas. A função aceita parâmetros da caixa de diálogo Adicionar Compromisso que é exibida quando você clica em Adicionar Compromisso no menu Addins no aplicativo de exemplo `AddAppointment` MFCMAPI.    A  `DisplayAddAppointmentDialog` função em Appointments.cpp exibe a caixa de diálogo e passa valores da caixa de diálogo para a  `AddAppointment` função. A função não está diretamente relacionada à criação de um item de compromisso usando  `DisplayAddAppointmentDialog` MAPI, portanto, ele não está listado aqui. 
  
> [!IMPORTANT]
> O código no aplicativo MFCMAPI não  garante que a pasta  Calendário tenha sido selecionada quando você clicar no comando Adicionar Compromisso no menu **Addins.** Criar um item de compromisso em uma pasta diferente da pasta **Calendário** pode causar um comportamento indefinido. Certifique-se de ter selecionado **a pasta Calendário** antes de usar o comando **Adicionar** Compromisso no aplicativo MFCMAPI. 
  
O  `AddAppointment` método está listado abaixo. Observe que o  _parâmetro lpFolder_ passado para o método é um ponteiro para uma  `AddAppointment` interface [IMAPIFolder](imapifolderimapicontainer.md) que representa a pasta onde o compromisso recorrente é criado. Dado o _parâmetro lpFolder_ que representa uma interface **IMAPIFolder,** o código chama o [método IMAPIFolder::CreateMessage.](imapifolder-createmessage.md) O **método CreateMessage** retorna um código de sucesso e um ponteiro para um ponteiro para uma interface **IMessage.** A maior parte do código da função trata do trabalho de especificação de propriedades em preparação para chamar o método `AddAppointment` [IMAPIProp::SetProps.](imapiprop-setprops.md) Se a chamada para o método **SetProps** for bem-sucedida, o método [IMAPIProp::SaveChanges](imapiprop-savechanges.md) será chamado para comprometer as alterações no armazenamento e criar um novo item de calendário. 
...
A  `AddAppointment` função usa várias funções auxiliares para criar uma estrutura para várias propriedades relacionadas a compromissos. As  `BuildTimeZoneStruct` funções  `BuildTimeZoneDefinition` auxiliares e auxiliares são usadas para criar uma estrutura que especifica as propriedades relacionadas ao fuso horário. As propriedades relacionadas ao fuso horário são **dispidTimeZoneStruct** ([PidLidTimeZoneStruct](pidlidtimezonestruct-canonical-property.md)), **dispidTimeZoneDesc** ([PidLidTimeZoneDescription](pidlidtimezonedescription-canonical-property.md)), **dispidApptTZDefRecur** ([PidLidAppointmentTimeZoneDefinitionRecur](pidlidappointmenttimezonedefinitionrecur-canonical-property.md)), **dispidApptTZDefStartDisplay** ([PidLidAppointmentTimeZoneDefinitionStartDisplay](pidlidappointmenttimezonedefinitionstartdisplay-canonical-property.md)) e **dispidApptTZDefEndDisplay** ([PidLidAppointmentTimeZoneDefinitionEndDisplay](pidlidappointmenttimezonedefinitionenddisplay-canonical-property.md)), e eles são discutidos nas seções correspondentes [de [MS-OXOCAL]](https://msdn.microsoft.com/library/cc425490%28v=EXCHG.80%29.aspx). 

A função é usada para criar uma estrutura que especifica as propriedades  `BuildGlobalObjectID` **LID_GLOBAL_OBJID** ([PidLidGlobalObjectId](pidlidglobalobjectid-canonical-property.md)) e **dispidCleanGlobalObjId** ([PidLidCleanGlobalObjectId](pidlidcleanglobalobjectid-canonical-property.md)), que são discutidas nas seções correspondentes [de [MS-OXOCAL]](https://msdn.microsoft.com/library/cc425490%28v=EXCHG.80%29.aspx). A estrutura que especifica a **propriedade dispidApptRecur** é criada usando a  `BuildWeeklyAppointmentRecurrencePattern` função. 

Para obter informações sobre a estrutura criada pela função, consulte a propriedade `BuildWeeklyAppointmentRecurrencePattern` [canônica PidLidAppointmentRecur.](pidlidappointmentrecur-canonical-property.md) Observe que, embora uma grande variedade de padrões de recorrência de compromissos seja possível, a função cria apenas um padrão de recorrência de compromisso  `BuildWeeklyAppointmentRecurrencePattern` semanal. Ele também usa vários valores codificados, como o tipo de calendário (Gregoriano), o primeiro dia da semana (domingo) e o número de instâncias modificadas ou excluídas (nenhum). Uma função de criação de padrão de recorrência de compromisso de finalidade mais geral precisaria aceitar esses tipos de variáveis como parâmetros. 
  
A seguir está a listagem completa da  `AddAppointment` função. 
