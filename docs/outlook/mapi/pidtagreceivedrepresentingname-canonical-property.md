...
Contém o nome de exibição do usuário de mensagens representado pelo usuário receptor.
...
Essas propriedades são exemplos das propriedades de endereço do usuário de mensagens que está sendo representado pelo usuário receptor. Eles devem ser definidos pelo provedor de transporte de entrada, que também é responsável pela autorização ou verificação do representante. Se nenhum usuário de mensagens estiver sendo representado, essas propriedades deverão ser definidas como o nome de exibição contido na **propriedade PR_RECEIVED_BY_NAME** ([PidTagReceivedByName](pidtagreceivedbyname-canonical-property.md)).
  
Um aplicativo cliente respondendo a uma mensagem recebida em nome de outro cliente deve copiar essas propriedades da mensagem recebida para a propriedade **PR_SENT_REPRESENTING_NAME** ([PidTagSentRepresentingName](pidtagsentrepresentingname-canonical-property.md)) da resposta.
