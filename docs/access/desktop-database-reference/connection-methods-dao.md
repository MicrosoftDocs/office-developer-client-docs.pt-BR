---
title: Métodos de Conexão (DAO)
TOCTitle: Methods
ms:assetid: d35831c7-083e-4acd-a18b-655db57b93f0
ms:mtpsurl: https://msdn.microsoft.com/library/Dn125610(v=office.15)
ms:contentKeyID: 52074484
ms.date: 09/18/2015
mtps_version: v=office.15
ms.openlocfilehash: 92b7c4bc4d91004c4400801e7d8307c62986d514
ms.sourcegitcommit: 38d0db57580cc5f4a0231c27b1643f8db5431ca3
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 11/02/2018
ms.locfileid: "25936396"
---
# <a name="connection-methods-dao"></a>Métodos de Conexão (DAO)

**Aplica-se a**: Access 2013, o Office 2013

- [Método Connection.Cancel (DAO)](connection-cancel-method-dao.md)
- [Método Close (DAO)](connection-close-method-dao.md)
- [Método Connection.CreateQueryDef (DAO)](connection-createquerydef-method-dao.md)
- [Método Execute (DAO)](connection-execute-method-dao.md)
- [Método Connection.OpenRecordset (DAO)](connection-openrecordset-method-dao.md)

