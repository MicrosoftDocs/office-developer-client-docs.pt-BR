---
title: Referência de coleção (ADOX) de modos de exibição
TOCTitle: Views Collection (ADOX)
ms:assetid: 7f92e594-160d-4e4d-afc4-e422c2b5b305
ms:mtpsurl: https://msdn.microsoft.com/library/JJ249542(v=office.15)
ms:contentKeyID: 48545907
ms.date: 09/18/2015
mtps_version: v=office.15
ms.openlocfilehash: f8163f4de9a59b2d27b2de3596669f6572529a5f
ms.sourcegitcommit: 19aca09c5812cfb98b68b5d4604dcaa814479df7
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 10/09/2018
ms.locfileid: "25462742"
---
# <a name="views-collection-adox-reference"></a>Referência de coleção (ADOX) de modos de exibição


**Aplica-se a**: Access 2013 | Office 2013



  - [Coleção Views (ADOX)](views-collection-adox.md)

  - [Propriedades da coleção Views, métodos e eventos (ADOX)](views-collection-properties-methods-and-events-adox.md)

