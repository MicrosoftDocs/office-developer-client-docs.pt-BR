...
Este exemplo utiliza o método [GetRows](getrows-method-ado.md) para recuperar todas as linhas da tabela *Custiomers* a partir de um [Recordset](recordset-object-ado.md) e preencher uma matriz com os dados resultantes. O método **GetRows** retornará menos que o número desejado de linhas em dois casos: se o [EOF](bof-eof-properties-ado.md) for alcançado ou se **GetRows** tentou recuperar um registro que foi excluído por outro usuário. A função retorna **False** apenas se o segundo caso ocorrer. Recorte e cole o seguinte código no Bloco de Notas ou em outro editor de texto e salve-o como **GetRowsJS.asp**.
