---
title: Coleção CubeDefs (ADO MD referência)
TOCTitle: CubeDefs Collection (ADO MD)
ms:assetid: ac77f45f-4e42-4d5c-9482-613188fe785b
ms:mtpsurl: https://msdn.microsoft.com/library/JJ249804(v=office.15)
ms:contentKeyID: 48547012
ms.date: 09/18/2015
mtps_version: v=office.15
ms.openlocfilehash: 62b88bc7a5693ad4faee089de536030f86e272d3
ms.sourcegitcommit: 19aca09c5812cfb98b68b5d4604dcaa814479df7
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 10/09/2018
ms.locfileid: "25463288"
---
# <a name="cubedefs-collection-ado-md-reference"></a>Coleção CubeDefs (ADO MD referência)


**Aplica-se a**: Access 2013 | Office 2013



  - [Coleção CubeDefs (ADO MD)](cubedefs-collection-ado-md.md)

  - [Propriedades da coleção CubeDefs, métodos e eventos (ADO MD)](cubedefs-collection-properties-methods-and-events-ado-md.md)

