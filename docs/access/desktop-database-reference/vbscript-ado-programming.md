...
No entanto, o ADO fornece dois arquivos que contêm as seguintes definições para serem usadas com o VBScript:
  ...
  - Para scripts do lado do servidor, use o Adovbs.inc, que é instalado na pasta ado c: Program \\ Files Common Files System \\ \\ \\ ado por \\ padrão.

  - Para scripts do lado do cliente, use o Adcvbs.inc, que é instalado na pasta \\ \\ \\ \\ msdac c: Program Files Common Files System \\ por padrão.

Você pode copiar e colar definições constantes desses arquivos em suas páginas ASP ou, se estiver fazendo scripts no lado do servidor, copiar o arquivo Adovbs.inc para uma pasta em seu site e referenciá-lo em sua página ASP da seguinte forma:
...
Exemplos mais específicos do VBScript encontram-se na documentação do ADO. Para obter mais informações, consulte [exemplos de código ADO no Microsoft Visual Basic Scripting Edition](ado-code-examples-in-microsoft-visual-basic-scripting-edition.md).
...
- Você não pode `on error goto <label>` usar no VBScript.
