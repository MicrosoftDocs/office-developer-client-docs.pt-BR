...
Vincula uma consulta de dados do [Recordset](recordset-object-ado.md) a um ou vários controles (por exemplo, uma caixa de texto, um controle de grade ou uma caixa de combinação) para exibir os dados do **Recordset** em uma página da Web.
...
Não há restrição de contagem no número de objetos **RDS.DataControl** que você pode inserir por meio de marcas OBJECT em uma única página da Web.

Quando você definir o objeto **RDS.DataControl** em uma página da Web, use os valores **Height** e **Width** diferentes de zero como 1 (para evitar a inclusão de espaços adicionais).
