---
title: Tópicos de exemplo de código do ADOX
TOCTitle: ADOX Code Examples
ms:assetid: ef8cc893-0484-4425-851d-0d65272fda99
ms:mtpsurl: https://msdn.microsoft.com/library/JJ250216(v=office.15)
ms:contentKeyID: 48548586
ms.date: 09/18/2015
mtps_version: v=office.15
ms.openlocfilehash: d992afb30b5f357bbfe62877f7d79a2bf3571a33
ms.sourcegitcommit: 19aca09c5812cfb98b68b5d4604dcaa814479df7
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 10/09/2018
ms.locfileid: "25464705"
---
# <a name="adox-code-example-topics"></a>Tópicos de exemplo de código do ADOX


**Aplica-se a**: Access 2013 | Office 2013


  - [Exemplos de código do ADOX](adox-code-examples.md)

  - [Exemplos de código do ADOX no Microsoft Visual Basic tópicos](adox-code-examples-in-microsoft-visual-basic-topics.md)

  - [Exemplos de código do ADOX no Microsoft Visual C++ tópicos](adox-code-examples-in-microsoft-visual-c-topics.md)

