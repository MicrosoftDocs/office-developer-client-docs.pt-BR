...
Se não houver nenhum registro atual, como quando não há nenhum registro no objeto **Recordset** , **AbsolutePosition** retorna – 1. Se o registro atual for excluído, o valor da propriedade **AbsolutePosition** não será definido, e ocorrerá um erro interceptável se ele for referenciado. Novos registros são adicionados ao final da sequência.
...
> - A propriedade **AbsolutePosition** não está disponível nos objetos **Recordset** do tipo somente encaminhamento ou nos objetos **Recordset** abertos a partir de consultas passagem nos Microsoft Access banco de dados conectados ao mecanismo bancos de dados ODBC.
