---
title: Objeto Level (ADO MD referência)
TOCTitle: Level Object (ADO MD)
ms:assetid: 376865c8-b5be-4dd3-8765-0469290e44ae
ms:mtpsurl: https://msdn.microsoft.com/library/JJ249131(v=office.15)
ms:contentKeyID: 48544191
ms.date: 09/18/2015
mtps_version: v=office.15
ms.openlocfilehash: 2ea3330a1598f0dddeac8c6a3a8aa5c62a0267e1
ms.sourcegitcommit: c557bbcccf37a6011f89aae1ddd399dfe549d087
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 10/31/2018
ms.locfileid: "25883698"
---
# <a name="level-object-ado-md-reference"></a>Objeto Level (ADO MD referência)


**Aplica-se a**: Access 2013, o Office 2013



  - [Objeto Level (ADO MD)](level-object-ado-md.md)

  - [Nível de propriedades, métodos e eventos (ADO MD)](level-properties-methods-and-events-ado-md.md)

