---
title: Referência de API do ActiveX Data Objects (ADO)
TOCTitle: ADO API reference
ms:assetid: 0fd6aff8-dbff-50c7-649f-2d9c31aedb2f
ms:mtpsurl: https://msdn.microsoft.com/library/JJ248867(v=office.15)
ms:contentKeyID: 48543275
ms.date: 09/18/2015
mtps_version: v=office.15
ms.openlocfilehash: 4fb6b06fd23577812f43db2f80829831b3f6409c
ms.sourcegitcommit: 48bfe5ab15b11105f4f52937b886c92bdc26525a
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 11/02/2018
ms.locfileid: "25910842"
---
# <a name="ado-api-reference"></a>Referência da API do ADO

**Aplica-se a**: Access 2013, o Office 2013

Esta seção da documentação do ActiveX Data Objects (ADO) contém tópicos para cada ADO objeto, coleção, propriedade, propriedade dinâmica, método, evento e enumeração. Além disso, contém uma lista de índices de sintaxe ADO a serem utilizados com o Microsoft Visual C++ e Windows Foundation Classes (WFC).

Para obter mais informações, procure por um tópico no índice ou consulte os seguintes tópicos:

- [Objetos ADO](ado-objects-and-interfaces.md)
- [Coleções ADO](ado-collections.md)
- [Propriedades do ADO](ado-properties.md)
- [Propriedades dinâmicas do ADO](ado-dynamic-properties.md)
- [Métodos do ADO](ado-methods.md)
- [Eventos do ADO](ado-events.md)
- [Constantes enumeradas do ADO](ado-enumerated-constants.md)
- [Exemplos de código do ADO](ado-code-examples.md)
- [Índices de sintaxe do ADO](https://docs.microsoft.com/office/vba/access/concepts/miscellaneous/ado-syntax-indexes)

