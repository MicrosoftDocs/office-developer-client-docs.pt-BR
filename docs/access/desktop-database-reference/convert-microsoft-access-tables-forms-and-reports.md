...
No Microsoft Access 2002 ou posterior, as caixas de combinação aceitam valores **Nulos** quando a propriedade **LimitToList** é definida como **True** (–1), independentemente de a lista conter valores **Null** ou não. Na versão 2.0, uma caixa de combinação com a propriedade **LimitToList** definida como **True** não aceitará um valor **Nulo** a não ser que a lista contenha um valor **Nulo**. Caso queira impedir que os usuários insiram um valor **Nulo** usando uma caixa de combinação, defina a propriedade **Required** do campo na tabela como **Sim**.

## <a name="menus-and-in-place-activation-of-ole-objects"></a>Menus e ativação in-place de objetos OLE

Para disponibilizar funcionalidade adicional ao ativar objetos OLE no local, alguns comandos de menu podem ter sido movidos para um menu que não é substituído quando você ativa um servidor OLE.
...
## <a name="date-fields-and-data-entry"></a>Campos de data e entrada de dados
...
## <a name="converted-version-20-report-has-different-margins"></a>O relatório da versão 2.0 convertido tem margens diferentes
...
## <a name="cant-use-the-format-property-to-distinguish-null-values-and-zero-length-strings"></a>Não é possível usar a propriedade Format para distinguir valores Null e cadeias de caracteres de comprimento zero
