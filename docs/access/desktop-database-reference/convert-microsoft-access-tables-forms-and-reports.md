...
No Microsoft Access 2002 ou posterior, as caixas de **** combinação aceitam valores nulos quando a propriedade **LimitToList** é definida como **true** (– 1), se a lista contém valores **nulos** . Na versão 2.0, uma caixa de combinação com a propriedade **LimitToList** definida como **True** não aceitará um valor **Nulo** a não ser que a lista contenha um valor **Nulo**. Caso queira impedir que os usuários insiram um valor **Nulo** usando uma caixa de combinação, defina a propriedade **Required** do campo na tabela como **Sim**.

## <a name="menus-and-in-place-activation-of-ole-objects"></a>Menus e ativação in-loco de objetos OLE

Para disponibilizar as funcionalidades adicionais para você ao ativar objetos OLE no lugar, alguns comandos de menu podem ter sido movidos para um menu que não é substituído quando você ativa um servidor OLE.
...
## <a name="cant-use-the-format-property-to-distinguish-null-values-and-zero-length-strings"></a>Não é possível usar a propriedade Format para distinguir valores nulos e cadeias de caracteres de comprimento zero
