---
title: Função Cast (aplicativo Web personalizado do Access)
manager: kelbow
ms.date: 09/05/2017
ms.audience: Developer
localization_priority: Normal
ms.assetid: b33f649c-d9b5-4ae5-a3fa-8681ed9a956d
description: Converte uma expressão de um tipo de dados para outro.
ms.openlocfilehash: c3f995aa59e95372bec448bf9e89c2c0616e36eb
ms.sourcegitcommit: 8fe462c32b91c87911942c188f3445e85a54137c
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 04/23/2019
ms.locfileid: "32280785"
---
# <a name="cast-function-access-custom-web-app"></a>Função Cast (aplicativo Web personalizado do Access)

Converte uma expressão de um tipo de dados para outro.
  
> [!NOTE]
> O recurso de armazenamento em nuvem descrito neste artigo não tem mais suporte no Office 2013 e Office 2016, e pode resultar no seguinte erro: > *Estamos com problemas de servidor, portanto, não podemos adicionar \<serviço\> agora. Tente novamente mais tarde.* > Para armazenamento em nuvem do Office Online, Office para iOS e Office para Android, você poderá pesquisar no nosso [Programa de Parceiros de Armazenamento de Nuvem do Office](https://dev.office.com/programs/officecloudstorage). 
  
## <a name="syntax"></a>Sintaxe

**Converter** (*Expressão*, *tipo de dados*) 
  
|**Nome do Argumento**|**Descrição**|
|:-----|:-----|
| *Expressão*  <br/> |Uma expressão válida.  <br/> |
| *DataType*  <br/> |O tipo de dados de destino.  <br/> |
   

