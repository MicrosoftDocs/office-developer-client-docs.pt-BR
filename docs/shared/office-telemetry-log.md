---
title: Log de telemetria do Office
manager: soliver
ms.date: 09/18/2015
ms.audience: Developer
localization_priority: Normal
ms.assetid: 179ba0c4-da24-430e-a212-0af40a604198
description: Encontre informações sobre o log de teleMetria incluído no Office 2013.
ms.openlocfilehash: bc408d0c42369b2132906d1df43ab64772a49489
ms.sourcegitcommit: 8657170d071f9bcf680aba50b9c07f2a4fb82283
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 04/28/2019
ms.locfileid: "33417452"
---
# <a name="office-telemetry-log"></a>Log de telemetria do Office

Encontre informações sobre o log de teleMetria incluído no Office 2013.
  
Explore esses links para saber mais sobre como usar o log de teleMetria do Office 2013 e interpretar os resultados que ele exibe.
  
## <a name="in-this-section"></a>Nesta seção

- [Selecionar problemas de arquivos do Office e soluções personalizadas com o log de telemetria](troubleshooting-office-files-and-custom-solutions-with-the-telemetry-log.md)
    
- [Compatibility issues in Office 2013](compatibility-issues-in-office.md)
    

